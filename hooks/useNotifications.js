...
export const useNotifications = () => {
    ...
    const registerForPushNotificationsAsync = async () => {
        ...
        // Check if we're on a physical device (simplified check)
    ...
    const scheduleStepReminder = async (targetTime = { hour: 20, minute: 0 }) => {
        ...
        try {
            ...
            const stepReminders = scheduledNotifications.filter(n =>
                n.content.data?.type === 'step-reminder'
            );
            ...
            console.log('Step reminder scheduled for', `${targetTime.hour}:${targetTime.minute}`);
        } catch (error) {
